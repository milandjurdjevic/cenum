...
internal class Enumeration
{
    ...
    public Enumeration(ITypeSymbol symbol)
    {
        _symbol = symbol;
    }
    ...
    {
        return field is { IsStatic: true, DeclaredAccessibility: Accessibility.Public, IsReadOnly: true } &&
               SymbolEqualityComparer.Default.Equals(field.Type, type);
    }

    public override string ToString()
    {
        using IndentedTextWriter writer = new IndentedTextWriter(new StringWriter());
        ...
        List<string> members = MembersOf(_symbol).ToList();
        if (members.Count > 0)
        {
